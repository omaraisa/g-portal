...
import uploadedLayersHandler, {allowedExtensions} from "../../modules/uploaded-layers-handler";
...
export default function AddUploadedLayer() {
  ...
  const [uploadInputRef, uploadLayerBtnRef, XFieldRef, YFieldRef] = [useRef(), useRef(), useRef(), useRef()];
  ...
  function toggleXYForm(status,fieldsNames) {
    setState({
      ...state,
      layerInfo: {...state.layerInfo,
      fieldsNames,
  ...
  function addfieldsNamesToSelectors(fieldsNames) {
    fieldsNames.forEach((field,index) => {
      ...
      if (lowerCaseField ===  'easting' || lowerCaseField === 'long' || lowerCaseField === 'lon' || lowerCaseField === 'x' || lowerCaseField ==='longitude' )
      XFieldRef.current.options[index+1].selected = true

      if (lowerCaseField === 'northing' || lowerCaseField === 'lat' || lowerCaseField === 'y' || lowerCaseField === 'latitude' )
  ...
  return (
    <div className="flex-column-container">
      <h3>رفع طبقة </h3>
      <span>xls, xlsx, txt, csv, json, geojson, kml, kmz, zipped Shapefile</span>
  ...
  return (
    <div className="flex-column-container">
      <i className="fas fa-arrow-circle-right backBtn" onClick={state => goBack(state)}></i>
      ...
      <button
        ...
        // ref={addLayerBtnRef}
        disabled={state.addLayerBtnDisabled}
        onClick={() => uploadLayer({state,xField:XFieldRef.current.value,yField:YFieldRef.current.value})}
      >
        إضافة الطبقة
      </button>
    </div>
