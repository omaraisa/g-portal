...
export default function AddUploadedLayer() {
  const [uploadBtnRef, addLayerBtnRef] = [useRef(), useRef()];
  ...
  return (
    <div className="flex-column-container">
      <h3>رفع طبقة </h3>
   <span>xls, xlsx, txt, csv, json, geojson, zipped Shapefile</span>
   ...
   <input
        ...
        ref={uploadBtnRef}
        onChange={() => layerChecker(uploadBtnRef.current)}
      />
      ...
      <button
        ...
        onClick={() => layerChecker(uploadBtnRef.current.value)}
      >
        رفع الطبقة
      </button>
